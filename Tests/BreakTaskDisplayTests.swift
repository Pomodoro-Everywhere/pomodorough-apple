...
struct BreakTaskDisplayTests {
    ...
    @Test func canonicalRetargetTaskFlowsThroughDisplay() throws {
    ...
    @Test func retargetedCompletionFollowsCanonicalHistory() throws {
        let oldTask = try #require(FocusTask(title: "Old task"))
        let newTask = try #require(FocusTask(title: "New task"))
        ...
        let terminal = TestFixtures.timer(
            ...
            timerID: timerID, taskID: newTask.id.uuidString.lowercased()
        )
        let snapshot = AppStatePublisher.Snapshot(
    ...
    @Test func unknownHistoryTaskFallsBackToNil() throws {
        let oldTask = try #require(FocusTask(title: "Old task"))
        let timer = TestFixtures.timer(
            status: .running, elapsed: 0, phase: .focus,
    ...
    @Test func breakTimerStaysHidden() throws {
        let oldTask = try #require(FocusTask(title: "Old task"))
        let newTask = try #require(FocusTask(title: "New task"))
        let timer = TestFixtures.timer(
